...
class ReadMeasurement_SFA3x():
    ...
    def Start_Measurement(self):
        ...
        print("Start_Measurement SN_64",SN_64)    

    def UpdateValues(self):
        global data_raw_64,data_location,SN_64
...
class GUI_DataShow():
    ...
    def Datachange(self):
        global data_raw_64,data_location,color_64,string_64,label_64,SN_64,SN_diff,Location_SameSN
        ...
        while True:
            ...
            print('SN_diff:', SN_diff)
            print('SN_64:', SN_64)
            print('Location_SameSN', Location_SameSN)
            print('Data_SameSN', Data_SameSN)
            #prepare the list with the same SN in the list
            for k in range(len(SN_diff)):
                print('now checn SN: ', SN_diff[k])
                for i in range(len(SN_64)): 
                    print('i',i)
                    print('data_location[i]', data_location[i])
            ...
            if len(data_location)>0: #avoid no any sensor connected 
                for i in range(len(Location_SameSN)): 
                    ...
                    for k in range(len(Data_SameSN[i])):
                        ...
                        # print('label_64', label_64)
                        # print(Location_SameSN[i][k])
                        if middian <=80:
...
for i in range(len(frame_64)):
    ...
    l = Label(frame_64[i], textvariable= my_string_var,font=(200), relief='ridge', bg='white') #display of values
